...
var posts = [], results = [], messages = [], index, target;
function action () {
...
function Search () {
    ...
    var content = document.getElementById("content");
    db.posts.find({"title": result.value}).toArray(function (err, doc) {
        if (err) throw err;
        results = doc;
        content.innerHTML = `
            ${(results.length == 0) ?
                ...
                `
                    ${results.map( result => `
                        <p><a onclick="self.location.href='#post/${result._id}'">${result.title}</a></p>
                    `).join("\n")}
                `
            }
        `
